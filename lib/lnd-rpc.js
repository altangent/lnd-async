...
function loadPackages({lndPath, creds, longsAsNumbers, allApi, output}) {
  for (let file in allApi) {
    let proto = allApi[file];

    const packageDefinition = loadSync(path.join(__dirname, 'original-lnd-protos', proto.file), {
      defaults: true,
      enums: String,
      keepCase: true,
      longs: longsAsNumbers ? Number : String,
      oneofs: true,
      includeDirs: path.join(__dirname, 'original-lnd-protos')
    });
    ...
    for (let serviceName in proto.services) {
      let service = new pkg[serviceName](lndPath, creds, options);
