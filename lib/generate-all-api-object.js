...
async function readFileOrDirectory(base, fileOrDir){
  if (fileOrDir.isDirectory()) {
    ...
    for await (const fileOrDir of dir) {
      readFileOrDirectory(newBase, fileOrDir)
    }
  }
  else {
    processLineByLine(path.join(base, fileOrDir.name))
  }
}
...
async function processLineByLine(file) {
  ...
  if (! pkg || ! service) {
    console.warn(`The file ${file} doesn't have a "package" or "service" directive`)
    if (pkg)
      delete output[file]
  }
}
